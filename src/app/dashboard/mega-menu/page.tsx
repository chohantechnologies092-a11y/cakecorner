...
export default async function MegaMenuSettingsPage() {
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--color-text-light, #666)", marginBottom: "2rem" }}>
        Select which categories should appear inside the "Cakes" mega menu dropdown in the top navigation bar.
      </p>

      <form action={updateSettings} style={{ background: "var(--color-background-glass, white)", padding: "2rem", borderRadius: "12px", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: "2px solid var(--color-border-glass, #eee)", textAlign: "left" }}>
          ...
          <tbody>
            {categories.map((cat: any) => (
              <tr key={cat.id} style={{ borderBottom: "1px solid var(--color-border-glass, #eee)" }}>
